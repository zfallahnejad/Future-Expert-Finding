...
public class DBM_ALL {
    ...
    public void startDBM_ALL() {
        for (String tag : Constants.TopTags) {
            for (int CurrentYear = 2008; CurrentYear < 2016; CurrentYear++) {
                PrintStream stdout = System.out;
                try {
                    PrintStream out = new PrintStream(new FileOutputStream(Constants.DBM_ALL_Directory + "DBM_All_" + tag + "_FYear_" + (CurrentYear+1) + ".txt"));
                    System.setOut(out);
                } catch (IOException e) {
                    System.out.println("\n\n\n\nSorry!\n\n\n\n");
                }
    ...
    private void getRelevantDocuments(String query, int cyear) {
        try {
            ...
            if (btQ != null) {
                ...
                Query q = u.BooleanQueryAnd(u.SearchCreationDateRange(2008, cyear),btQ);
